	...
	static LoadContext getPlatform(CommandAPIConfig<?> config) {
		...
		try {
			...
			PaperNMS<?> versionAdapter = switch (version) {
				...
				case "1.21.9", "1.21.10", "1.21.11" -> new PaperNMS_1_21_R6();
				default -> null;
			};
				...
				return new LoadContext(
						...
						() -> {
							...
							CommandAPI.logWarning(
									"Loading the CommandAPI with the latest and potentially incompatible NMS implementation.");
							CommandAPI.logWarning(
									"While you may find success with this, further updates might be necessary to fully support the version you are using.");
						});
			}
			...
			if (error instanceof ClassNotFoundException cnfe) {
				// Thrown when users use versions before the ServerBuildInfo was added. We
				// should inform them
